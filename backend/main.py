...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.executemany("INSERT INTO communities (name, description, category, member_count) VALUES (?, ?, ?, ?)", communities)
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.executemany("INSERT INTO events (title, date, timing, venue, status, community_id) VALUES (?, ?, ?, ?, ?, ?)", events)
...
async def login(request: LoginRequest):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, email, role FROM users WHERE email = ? AND password = ?", (request.email, request.password))
    user = cursor.fetchone()
    conn.close()
    if user:
        return {"success": True, "id": user["id"], "name": user["name"], "email": user["email"], "role": user["role"]}
    else:
...
async def get_events():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT e.id, e.title, e.date, e.timing, e.venue, e.status, e.community_id, c.name as community_name
        FROM events e LEFT JOIN communities c ON e.community_id = c.id
...
async def add_event(request: EventRequest):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO events (title, date, timing, venue, status, community_id) VALUES (?, ?, ?, ?, ?, ?)",
...
async def create_join_request(request: JoinRequest):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO join_requests (community_id, full_name, enrollment_no, branch, semester, reason, status) VALUES (?, ?, ?, ?, ?, ?, 'pending')",
                   (request.community_id, request.full_name, request.enrollment_no, request.branch, request.semester, request.reason))
    conn.commit()
    conn.close()
...
async def get_recruitment(email: str = ""):
    ...
    if email:
        cursor.execute("""SELECT r.*, c.name as community_name FROM recruitment r
            LEFT JOIN communities c ON r.community_id = c.id
            WHERE r.student_email = ? AND r.expires_at > ? ORDER BY r.announced_at DESC""", (email, now))
    else:
        cursor.execute("""SELECT r.*, c.name as community_name FROM recruitment r
            LEFT JOIN communities c ON r.community_id = c.id
            WHERE r.expires_at > ? ORDER BY r.announced_at DESC""", (now,))
...
async def add_recruitment(request: RecruitmentRequest):
    ...
    cursor.execute("INSERT INTO recruitment (student_email, student_name, community_id, status, expires_at) VALUES (?, ?, ?, ?, ?)",
                   (request.student_email, request.student_name, request.community_id, request.status, expires_at))
    conn.commit()
    ...
    if user and community:
        ...
        notif_message = f"You have been {status_text} for {community_name}. " + ("Welcome to the team!" if request.status == "selected" else "Better luck next time!")
        notif_type = "success" if request.status == "selected" else "error"
        cursor.execute("INSERT INTO notifications (user_id, role, title, message, type, expires_at) VALUES (?, 'student', ?, ?, ?, ?)",
                       (user["id"], notif_title, notif_message, notif_type, expires_at))
        cursor.execute("SELECT id FROM users WHERE role = 'admin'")
        admins = cursor.fetchall()
        for admin in admins:
            cursor.execute("INSERT INTO notifications (user_id, role, title, message, type, expires_at) VALUES (?, 'admin', ?, ?, 'info', ?)",
                           (admin["id"], f"Recruitment Update — {community_name}", f"{request.student_name} has been marked as {status_text} for {community_name}.", expires_at))
    conn.commit()
    conn.close()
...
async def get_notifications(user_id: int):
    ...
    cursor = conn.cursor()
    now = datetime.now().isoformat()
    cursor.execute("SELECT * FROM notifications WHERE user_id = ? AND expires_at > ? ORDER BY created_at DESC", (user_id, now))
...
async def create_notification(request: NotificationRequest):
    ...
    cursor = conn.cursor()
    expires_at = (datetime.now() + timedelta(days=request.expires_days)).isoformat()
    cursor.execute("INSERT INTO notifications (user_id, role, title, message, type, expires_at) VALUES (?, ?, ?, ?, ?, ?)",
                   (request.user_id, request.role, request.title, request.message, request.type, expires_at))
    conn.commit()
    conn.close()
